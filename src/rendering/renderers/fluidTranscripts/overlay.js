 ...
 */
export function injectFluidTranscriptsFacsimileLayers ({ fluidSvg, overlayContext, positionalData, dtDom, sourceDom, parser, logger }) {
  if (!fluidSvg || !overlayContext?.facsimile) return

  const { xOffsetUnits, yOffsetUnits, unitsPerMm } = resolveFluidTranscriptsOverlayPlacement({
    overlayContext,
    positionalData,
    fluidSvg,
    dtDom,
