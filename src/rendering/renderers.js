...
/**
 * Resolves facsimile and shapes overlay context for one DT page.
 *
 ...
 */
function resolveFluidSystemsOverlayContext ({ dtDom, sourceDom, triple }) {
...
/**
 * Appends a simple animate element with shared fluidSystems timing defaults.
 *
...
/**
 * Injects facsimile image and writing-zone shapes overlays plus metadata.
 *
 * @param {Object} params - Input parameter bundle.
 * @param {SVGElement|Document} params.fluidSvg - Fluid systems SVG document.
 ...
 * @returns {void} No return value.
 */
function injectFluidSystemsFacsimileLayers ({ fluidSvg, overlayContext, parser, logger }) {
  if (!fluidSvg || !overlayContext?.facsimile) return
  ...
  if (overlayContext.facsimile.fragment.rotate !== 0) {
    ...
    image.setAttribute('transform', `rotate(${overlayContext.facsimile.fragment.rotate} ${rotateCenterX} ${rotateCenterY})`)
  }
  ...
  if (shapesPath && fs.existsSync(shapesPath)) {
    ...
    if (overlayContext.facsimile.fragment.rotate !== 0) {
      const rotateCenterX = (overlayContext.facsimile.mediaFragMm.x + (overlayContext.facsimile.mediaFragMm.w / 2)) * unitsPerMm
      const rotateCenterY = (overlayContext.facsimile.mediaFragMm.y + (overlayContext.facsimile.mediaFragMm.h / 2)) * unitsPerMm
      shapesWorld.setAttribute('transform', `rotate(${overlayContext.facsimile.fragment.rotate} ${rotateCenterX} ${rotateCenterY})`)
    }
    ...
    logger.warn(`[renderFluidSystemsSvg] Shapes SVG not found: ${shapesPath}`)
  }
...
export async function renderFluidSystemsSvg ({ data, triple, verovio, pageDimensions, recreate, logger }) {
  ...
  try {
    ...
    if (overlayContext) {
      ...
      logger.warn('[renderFluidSystemsSvg] Could not resolve facsimile/shapes overlay context; writing transcription-only output.')
    }
